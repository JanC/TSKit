...
void onTalkStatusChangeEvent(uint64 serverConnectionHandlerID, int status, int isReceivedWhisper, anyID clientID) {
	...
	int error;
    /* Query client nickname from ID */
	if((error = ts3client_getClientVariableAsString(serverConnectionHandlerID, clientID, CLIENT_NICKNAME, &name)) != ERROR_ok) {
		printf("ts3client_getClientVariableAsString %i\n", error);
		return;		
	}

	uint64 clientChannelID;
    error = ts3client_getChannelOfClient(serverConnectionHandlerID, clientID, &clientChannelID);
    if(error != ERROR_ok) {
        printf("Could not get channel of client %i\n", error);
    }

    char *channel_name;
    if ((error = ts3client_getChannelVariableAsString(serverConnectionHandlerID, clientChannelID, CHANNEL_NAME, &channel_name)) != ERROR_ok) {
    	printf("ts3client_getChannelVariableAsString %i\n", error);
    }

   

    if(status == STATUS_TALKING) {
        printf("Client \"%s\" starts talking in %s\n", name, channel_name);
    } else {
        printf("Client \"%s\" stops talking in %s\n", name, channel_name);
    }
	ts3client_freeMemory(name); 
	ts3client_freeMemory(channel_name);
}
...
int main(int argc, char* argv[]) {
    ...
    short abort = 0;
	char* path;
	char* clientPassword = "1234";
	char* clientName = "c-client";
	...
	if (argc == 6) {
		...
		connect_info.port = 9986;
	}

	printf("##### Connecting to: %s:%d (pw: %s) transmitting: %d ##### \n", connect_info.ip, connect_info.port, clientPassword, transmit);


	int channelID = 0;
	if(argc == 6) {
		channelID =  atoi(argv[5]);
	}
	...
	char*** capture_devices = NULL;
    if(ts3client_getCaptureDeviceList(mode, &capture_devices) == ERROR_ok) {
		//printf("ts3client_getCaptureDeviceList: %s\n", capture_devices[1][0]);

        int len = sizeof(capture_devices)/sizeof(capture_devices[0]);
        for(int i=0; i <= len; i++) {
            char** current_device = capture_devices[i];
	...
	if((error = ts3client_getDefaultCaptureDevice(mode, &device)) != ERROR_ok) {
	 ...
	 if(channelID != -1) {
     	device = capture_devices[channelID];
     }
    ...
    //char * clientName = "PilotEdge_Receiver_Mac"
    const char *channels[2] = { "17510", "\0" };

	error = ts3client_startConnection(scHandlerID, identity, connect_info.ip, connect_info.port, clientName, channels, "", clientPassword);
