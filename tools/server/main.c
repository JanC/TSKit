...
void createChannelPass(uint64 serverID, const char *name, const char *password) {
	unsigned int error;
	uint64 newChannelID;
...
void createChannel(uint64 serverID, const char *name) {
	createChannelPass(serverID, name, NULL);
}
...
int main() {
    ...
    serverID = createVirtualServer("TS3 SDK Test Server", 9986, MAX_CLIENTS);
    ...
    /* Set server password */
    if((error = ts3server_setVirtualServerVariableAsString(serverID, VIRTUALSERVER_PASSWORD, "1234")) != ERROR_ok) {
